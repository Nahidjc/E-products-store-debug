...
function showSearchProducts(inputText) {
  document.getElementById("all-products").innerHTML = ''
  const inputTextUpper = inputText.toUpperCase()

  console.log(inputTextUpper);
  const findProducts = data.filter(d => d.title.toUpperCase().includes(inputTextUpper))
  showProducts(findProducts);
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <p>Rating: <span class="rating"><i class="far fa-star"></i> ${product.rating.rate}</span>   votes: <span class="rating-count"> ${product.rating.count} </span></p>
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn addcart"><i class="fas fa-cart-plus"></i>  add to cart</button>
...
const getInputValue = (id) => {
  ...
  const fixedPrice = converted.toFixed(2);
  console.log(parseFloat(fixedPrice));
  return converted;
};
...
const updateTotal = () => {
  ...
  console.log(price, deliveryCharge, totalTax);
